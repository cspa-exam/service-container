    ...
    static parse(func) {
        ...
        switch (node.type) {
            case 'ClassDeclaration': {
                ...
                if (!node.id) {
                    throw new DependencyInjectionError_1.DependencyInjectionError('function_parser_no_function_id', `Class declaration must have a name: [${func.toString()}]`);
                }
                return {
                    name: node.id.name,
                    type: 'class',
                    is_subclassed: !!(node.superClass && node.superClass.name),
            ...
            case 'FunctionDeclaration': {
                if (!node.id) {
                    throw new DependencyInjectionError_1.DependencyInjectionError('function_parser_no_function_id', `Function declaration must have a name: [${func.toString()}]`);
                }
                return {
                    name: node.id.name,
                    type: 'function',
            ...
            default: {
                throw new DependencyInjectionError_1.DependencyInjectionError('function_parser_invalid_ast_body_type', 'Input was neither a valid function or class declaration?');
            }
        }
        function getArgsFromParams(params) {
            return params.map(_param => {
                if ('AssignmentPattern' === _param.type) {
                    ...
                    throw new DependencyInjectionError_1.DependencyInjectionError('function_parser_unsupported_parameter', `Unsupported parameter syntax in [${func.toString()}]`);
                }
            }) || [];
        }
    }
        ...
        const result = (() => {
            if ('class' === parse.type) {
                if (!parse.has_constructor) {
                    throw new DependencyInjectionError_1.DependencyInjectionError('function_parser_class_missing_constructor', 'Class is missing a constructor.');
                }
                ...
                throw new DependencyInjectionError_1.DependencyInjectionError('function_parser_constructor_invalid_ast_type', 'Input was neither a valid function or class declaration?');
            }
        })();
